...
test("room shell supports experimental room theme classes without changing the default skin", () => {
  ...
  assert.match(wxss, /\.page\.room-screen \.room-self__die\s*\{/);
  assert.doesNotMatch(wxss, /\.page\.room-screen \.seat-stage-cup \.figma-cup__rim/);
...
test("room self rolling animation uses staged motion, reveal sequencing, and dedicated die slots", () => {
  ...
  assert.match(wxss, /\.room-self__dice-cup\.is-rolling\s*\{[\s\S]*animation:\s*room-self-cup-rock 260ms cubic-bezier\(0\.34,\s*0\.02,\s*0\.18,\s*1\) infinite/);
  assert.match(wxss, /\.room-self__dice-cup\.is-rolling \.room-self__dice-stack\s*\{[\s\S]*animation:\s*room-self-stack-shift 210ms cubic-bezier\(0\.32,\s*0,\s*0\.18,\s*1\) infinite/);
  ...
  assert.match(wxss, /@keyframes room-self-cup-rock\s*\{[\s\S]*25%\s*\{[\s\S]*rotate\(-14deg\) scale\(1\.015\)/);
  assert.match(wxss, /@keyframes room-self-cup-settle\s*\{[\s\S]*0%\s*\{[\s\S]*translate\(12rpx,\s*16rpx\) rotate\(-7deg\)/);
  assert.match(wxss, /@keyframes room-self-stack-shift\s*\{[\s\S]*35%\s*\{[\s\S]*translate\(20rpx,\s*-7rpx\) rotate\(10deg\) scale\(1\.08\)/);
  assert.match(wxss, /@keyframes room-self-die-spin\s*\{[\s\S]*rotate\(-18deg\) scale\(0\.88\)/);
  assert.match(wxss, /@keyframes room-self-die-spin-reveal\s*\{[\s\S]*30%\s*\{[\s\S]*rotate\(64deg\) scale\(1\.04\)/);
  assert.match(wxss, /@keyframes room-self-die-spin\s*\{[\s\S]*75%\s*\{[\s\S]*rotate\(236deg\) scale\(1\.08\)/);
  assert.match(wxss, /@keyframes room-self-die-spin\s*\{[\s\S]*100%\s*\{[\s\S]*rotate\(318deg\) scale\(0\.94\)/);
  assert.match(wxss, /@keyframes room-self-die-settle\s*\{[\s\S]*transform:\s*translateY\(20rpx\) scale\(0\.86\)/);
