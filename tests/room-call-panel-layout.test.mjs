...
test("room call panel opens manually during calling and supports overlay close", () => {
  ...
  assert.match(roomWxml, /class="room-fab \{\{phase === 'rolling' && secondaryActionKind === 'reroll' \? 'room-fab--left room-fab--paired' : ''\}\} \{\{canPrimaryAction \? '' : 'is-disabled'\}\} \{\{primaryActionText === '叫牌' \? roomThemeAssets\.primaryButtonClass : ''\}\} \{\{roomThemeId === 'ruby-red' && primaryActionText && primaryActionText !== '叫牌' \? 'room-fab--ruby-open-slice' : ''\}\}"[\s\S]*hover-class="\{\{canPrimaryAction \? 'room-fab--pressing' : 'none'\}\}"/);
