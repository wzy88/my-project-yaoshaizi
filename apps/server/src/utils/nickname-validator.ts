...
export function normalizeNicknameInput(raw: unknown): string {
  return String(raw ?? "")
    .replace(/[\u0000-\u001f\u007f-\u009f\u200b-\u200f\uFEFF]/g, "")
    .replace(/\s+/g, " ")
    .trim();
...
export function validateNicknameInput(raw: unknown): { ok: true; value: string } | { ok: false; value: string; message: string } {
...
export function assertValidNickname(raw: unknown): string {
  const result = validateNicknameInput(raw);
  if (!result.ok) {
    throw new GameError(ErrorCode.BAD_REQUEST, result.message);
  }
